...
function printHelp() {
  console.log(`
${bold("nukport")} \u2014 kill whatever is running on a port

${bold("USAGE")}
  nukport ${cyan("<port>")} [options]
  nukport ${cyan("<port1> <port2> ...")}  (multiple ports)
  nukport ${cyan("--list")}
...
function run() {
  ...
  if (ports.length === 0) {
    ...
    console.error(dim("Usage: nukport <port> [--force] [--dry-run]"));
    process.exit(1);
  }
